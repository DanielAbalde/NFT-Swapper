...
contract ERC20Handler is ISwapperHandler {
    ...
    function isHandlerOf(address tokenContract) public view override returns (bool){
      bool success;
      bytes memory data;
      (success, data) = tokenContract.call(abi.encodeWithSignature("supportsInterface(uint256)", type(IERC20).interfaceId));
      if(success) {
        return abi.decode(data, (bool));
      }else{
        (success, data) = tokenContract.call(abi.encodeWithSignature("totalSupply()"));
        if(!success) {
          return false;
        }
        (success, data) = tokenContract.call(abi.encodeWithSignature("balanceOf(address)", msg.sender));
        if(!success) {
          return false;
        }
        return true;
